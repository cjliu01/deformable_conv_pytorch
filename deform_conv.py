...
class DeformConv2D(nn.Module):
    def __init__(self, inc, outc, kernel_size=3, stride=1, padding=1, bias=None, modulation=False):
        ...
        self.offsets = nn.Conv2d(inc, 2 * kernel_size * kernel_size, kernel_size=kernel_size, stride=stride)
        nn.init.constant_(self.offsets.weight, 0)
        self.modulation = modulation
        if modulation:
            self.weights = nn.Conv2d(inc, kernel_size * kernel_size, kernel_size=3, stride=stride)
    ...
    def forward(self, x):
        ...
        if self.modulation:
            m = torch.sigmoid(self.weights(x))
        ...
        q_lt = torch.cat([torch.clamp(q_lt[..., :N], 0, x.size(3) - 1), torch.clamp(q_lt[..., N:], 0, x.size(2) - 1)],
                         dim=-1).long()
        q_rb = torch.cat([torch.clamp(q_rb[..., :N], 0, x.size(3) - 1), torch.clamp(q_rb[..., N:], 0, x.size(2) - 1)],
                         dim=-1).long()
        q_lb = torch.cat([q_lt[..., :N], q_rb[..., N:]], -1)
        q_rt = torch.cat([q_rb[..., :N], q_lt[..., N:]], -1)

        # (b, h, w, N)
        p = torch.cat([torch.clamp(p[..., :N], 0, x.size(3) - 1), torch.clamp(p[..., N:], 0, x.size(2) - 1)], dim=-1)
    ...
    def _get_p_n(self, N, dtype):
        ...
        # (2N, 1)
        p_n = torch.concat((p_n_x.flatten(), p_n_y.flatten()), dim=0)
    ...
    def _get_p_0(self, h, w, N, dtype):
        p_0_x, p_0_y = torch.meshgrid(
            torch.arange(1, h * self.stride + 1),
            torch.arange(1, w * self.stride + 1), indexing='ij')
